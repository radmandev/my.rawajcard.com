...
function NavPills({ dark = false, compact = false }) {
 ...
 dark
 ?'border-indigo-700 text-slate-200'
 :'border-slate-300 text-slate-700 bg-indigo-950/60'
...
function CtaPair({ dark = false }) {
 return (
 <div className="flex items-center gap-2 flex-wrap">
 <button className="shrink-0 px-3 py-2 rounded-lg bg-cyan-600 text-white text-xs md:text-sm whitespace-nowrap">Buy NFC Card</button>
 <button className={`shrink-0 px-3 py-2 rounded-lg text-xs md:text-sm whitespace-nowrap ${dark ?'bg-indigo-950/60 text-slate-900' :'bg-slate-900 text-white'}`}>
 Create Digital Card
 </button>
 </div>
 );
...
function VariantCard({ id, title, selected, onSelect, children }) {
 return (
 <section className="rounded-2xl border border-slate-200 overflow-hidden bg-indigo-950/60 shadow-sm">
 <div className="flex items-center justify-between px-4 py-3 bg-slate-50 border-b border-slate-200">
 <h2 className="font-semibold text-slate-800">v{id} — {title}</h2>
 <button
 onClick={() => onSelect(id)}
...
export default function HeaderVariants() {
 ...
 <div className="rounded-2xl bg-indigo-950/60 border border-slate-200 p-5 md:p-6">
 <h1 className="text-2xl md:text-3xl font-bold text-slate-900">Temporary Header Lab</h1>
 <p className="mt-2 text-slate-600">
 10 new no-shrink variants (all header items fit using wrap/stack/scroll). Current pick: <span className="font-semibold text-cyan-600">Version {selectedVersion}</span>
 </p>
 </div>

 <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
 <VariantCard id={1} title="Two Rows Balanced" selected={selectedVersion === 1} onSelect={setSelectedVersion}>
 <header className="rounded-xl bg-indigo-950/60 border border-slate-200 p-3 space-y-3">
 ...
 <header className="rounded-xl bg-indigo-950/60 border border-slate-200 p-3 grid grid-cols-1 lg:grid-cols-[auto_1fr_auto] gap-3 items-center">
 <Brand />
 ...
 <header className="rounded-xl overflow-hidden border border-slate-200 bg-indigo-950/60">
 ...
 <header className="rounded-xl bg-indigo-950/60 border border-slate-200 p-3">
 ...
 <div className="rounded-xl bg-indigo-950/60/10 p-2 overflow-x-auto">
 <div className="min-w-max">
 <NavPills dark />
 </div>
 </div>
 ...
 <header dir="rtl" className="rounded-xl bg-indigo-950/60 border border-slate-200 p-3 space-y-3">
 <div className="flex items-center justify-between gap-3 flex-wrap">
 ...
 <span key={item} className="shrink-0 rounded-full border border-slate-300 px-3 py-1 text-xs md:text-sm whitespace-nowrap bg-indigo-950/60 text-slate-700">{item}</span>
 ))}
 </nav>
 <div className="flex items-center gap-2 flex-wrap">
 <button className="shrink-0 px-3 py-2 rounded-lg bg-cyan-600 text-white text-xs md:text-sm whitespace-nowrap">اشتر بطاقة NFC</button>
 ...
 <header className="rounded-xl bg-slate-900 border border-cyan-400/30 shadow-[0_0_30px_rgba(56,189,248,0.15)] p-3 space-y-3">
 <div className="flex items-center justify-between gap-3 flex-wrap">
 <Brand dark />
 <UtilityChips dark />
 </div>
 ...
 <header className="rounded-xl border border-slate-200 bg-indigo-950/60 p-3 space-y-3">
