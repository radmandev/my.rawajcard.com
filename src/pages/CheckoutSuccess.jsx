...
export default function CheckoutSuccess() {
 ...
 useEffect(() => {
 if (successType ==='order' && orderNumber) {
 const dedupeKey =`rawaj_site_purchase_${orderNumber}`;
 if (sessionStorage.getItem(dedupeKey)) return;
 sessionStorage.setItem(dedupeKey,'1');

 void trackWebsiteEvent('purchase', {
 ...
 if (successType ==='subscription' && activatedPlan) {
 const dedupeKey =`rawaj_site_subscription_${activatedPlan}`;
 if (sessionStorage.getItem(dedupeKey)) return;
 sessionStorage.setItem(dedupeKey,'1');

 void trackWebsiteEvent('subscription_purchase', {
 pageName:'Checkout Success',
 path:'/CheckoutSuccess',
 metadata: {
 plan: activatedPlan,
 ...
 }, [activatedPlan, orderNumber, orderValue, paymentMethod, successType]);
 ...
 >
 <Card className="bg-indigo-950/60 border-cyan-200/50">
