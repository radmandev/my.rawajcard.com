...
export default function MyOrders() {
 ...
 <span className="font-mono text-xs font-semibold text-slate-700">#{number}</span>
 <span className="text-xs text-slate-400">{items.length} {isRTL ?'منتج' :'items'}</span>
 </div>
 <p className="mt-2 text-sm font-medium text-slate-900 line-clamp-1">
 ...
 <div className="h-14 w-14 rounded-lg overflow-hidden bg-slate-100 flex items-center justify-center flex-shrink-0">
 {item.product_image ? (
 ...
 {hasCustomization && (
 <p className="text-xs text-cyan-600 mt-1">{isRTL ?'تم الحفظ' :'Saved'}</p>
 )}
 </div>
 </div>
 </button>
 ...
 <label className="w-full cursor-pointer rounded-xl border-2 border-dashed border-slate-300 p-4 flex items-center justify-center gap-2 text-sm text-slate-600 hover:border-cyan-500 transition-colors">
 {picture ? (
 <span className="text-cyan-600 font-medium">{isRTL ?'✓ تم رفع الصورة — اضغط لتغييرها' :'✓ Photo uploaded — click to change'}</span>
 ) : (
 <>
