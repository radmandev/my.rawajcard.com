...
function StatCard({ icon: Icon, label, value, sub = null, trend = null, trendVal = null, color = TEAL }) {
 ...
 {(isUp || isDown) && (
 <div className={`flex items-center gap-1 mt-3 text-xs font-medium ${isUp ?'text-emerald-400' :'text-red-500'}`}>
...
function RealTimeBar({ count, isRTL }) {
 return (
 <div className="flex items-center gap-2 bg-emerald-500/15 border border-emerald-400/30 rounded-xl px-4 py-2">
...
function EventTable({ events, isRTL }) {
 ...
 <code className="text-xs bg-white/10 text-cyan-300 px-2 py-0.5 rounded-md">
 {e.event}
 </code>
 </td>
 ...
 {e.trend ==='up' && <span className="text-emerald-400 text-xs font-medium flex items-center justify-end gap-0.5"><TrendingUp className="w-3 h-3" /> Up</span>}
...
function ExportButton({ label, onClick }) {
 ...
 <button
 onClick={onClick}
 className="flex items-center gap-2 px-3 py-2 text-xs font-semibold text-cyan-300 bg-cyan-500/15 hover:bg-cyan-500/25 rounded-xl transition-colors border border-cyan-400/30"
 >
