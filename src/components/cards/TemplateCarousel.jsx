...
export default function TemplateCarousel({ selectedTemplate, onSelect, onFocusChange, userPlan ='free', onUpgrade }) {
 ...
 className="absolute -left-4 top-1/2 -translate-y-1/2 z-20 h-10 w-10 rounded-full bg-indigo-950/60 shadow-lg flex items-center justify-center hover:bg-slate-50 transition-colors"
 >
 <ChevronLeft className="h-5 w-5" />
 </button>
 <button onClick={handleNext}
 className="absolute -right-4 top-1/2 -translate-y-1/2 z-20 h-10 w-10 rounded-full bg-indigo-950/60 shadow-lg flex items-center justify-center hover:bg-slate-50 transition-colors"
 >
...
'w-64 rounded-xl overflow-hidden border-2 transition-all',
 ...
 <div className="aspect-[3/4] bg-indigo-950/60 overflow-hidden relative">
...
'h-2 rounded-full transition-all',
 index === currentIndex
 ? isPremium ?'w-8 bg-amber-500' :'w-8 bg-cyan-600'
 :'w-2 bg-slate-300'
 )}
 />
 );
