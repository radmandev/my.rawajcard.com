...
export default function DraggableColorPicker({ colors, onColorsChange, isRTL }) {
 ...
 className={`flex items-center gap-3 p-3 bg-slate-50 rounded-lg border-2 transition-all ${
 snapshot.isDragging 
 ...
 className="h-10 w-16 rounded cursor-pointer border-2 border-slate-200"
 />
 <Input
 value={color.value}
 onChange={(e) => handleColorChange(color.id, e.target.value)}
