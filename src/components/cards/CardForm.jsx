...
export default function CardForm({ card, onChange }) {
 ...
 className="p-1 bg-cyan-600 rounded-full text-white hover:bg-cyan-700"
 ...
 disabled={uploadingField ==='cover_image'}
 onClick={() => coverImageRef.current?.click()}
 className="w-full h-24 rounded-lg border-2 border-dashed border-slate-200 flex items-center justify-center hover:border-cyan-400 transition-colors disabled:opacity-50"
 >
 <div className="text-center">
 {uploadingField ==='cover_image' ? (
 <Loader2 className="h-6 w-6 mx-auto text-cyan-500 animate-spin mb-1" />
 ) : (
 ...
 colors={{
 primary: card.design?.primary_color ||'#38BDF8',
 secondary: card.design?.secondary_color ||'#1E1B4B',
 accent: card.design?.accent_color ||'#E879F9'
 }}
